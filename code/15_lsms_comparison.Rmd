...
```{r}
lsms_table <- lsms_estimates %>% 
  filter(!(iso3 == "NER" & survey_year == 2021)) %>% 
  filter(internet_n_sample_f > 150) %>% 
  group_by(iso3, survey_year) %>% 
  summarize(female_sample = sum(internet_n_sample_f), male_sample = sum(internet_n_sample_m), subnational_units = n()) %>% 
  mutate(country = countrycode::countrycode(iso3, origin = "iso3c", destination = "country.name")) %>% 
  mutate(country_code = countrycode::countrycode(iso3, origin = "iso3c", destination = "iso2c")) %>% 
  ungroup() %>% 
  select(country, country_code, survey_year, female_sample, male_sample)
