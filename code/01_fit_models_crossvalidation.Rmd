...
gadm1 <- read_csv(here("data",  "aggregated_indicators_lmic_aug13.csv"), locale = locale(encoding = "ISO-8859-1")) 

## read in covariates  

gadm1 <- gadm1 %>% 
  dplyr::select(country, gid_0, gid_1, perc_used_internet_past12months_wght_age_15_to_49_wom, continent, all_of(c(dhs_vars, fb_feats, off_feats))) %>% ## the vectors of vars are defined in helper function
  mutate(continent = case_when(continent == "Africa" ~ 1,
                               TRUE ~ 0)) %>%
  drop_na(dhs_vars) %>% #off_feats
...
## impute missing values as median (only necessary f or a handful of cases)
gadm1 <- gadm1 %>% 
...
gadm1 <- gadm1 %>% 
  mutate(fb_pntr_13p_female_dhsyear_national = case_when(
    fb_pntr_13p_female_dhsyear_national > 1.1 ~ 1.1,
    TRUE ~ fb_pntr_13p_female_dhsyear_national
  )) %>% 
  mutate(fb_pntr_13p_male_dhsyear_national = case_when(
    fb_pntr_13p_male_dhsyear_national > 1.1 ~ 1.1,
    TRUE ~ fb_pntr_13p_male_dhsyear_national
  ))
...
gadm1 %>%
  ...
  filter(!is.na(fb_pntr_13p_male_dhsyear_impute)) %>%
  summarize(cor = cor(perc_used_internet_past12months_wght_age_15_to_49_wom, fb_pntr_13p_male_dhsyear_impute),
            n()) %>% arrange(cor)
```
