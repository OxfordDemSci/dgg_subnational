...
master_file <- fread(here("data", "master_data_file_march12_with_national.csv")) %>%
  filter(!is.na(perc_used_internet_past12months_wght_age_15_to_49_wom)) %>% 
  drop_na() %>% 
  filter(year != 2024)

master_file_full <- fread(here("data", "master_data_file_march12_with_national.csv")) %>%
...
```{r}
## read in predictions
predictions_wide <- fread(here("out", "predictions_wide.csv")) 
