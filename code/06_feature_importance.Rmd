...
r2_plot <- model_performance_metrics %>% 
  ...
  ggplot(aes(x = reorder(model, -value), y = value, fill = cv_method, label = scales::percent(value, accuracy = 1))) + 
  ...
  facet_grid(rows = vars(outcome_clean), cols = vars(feature_set)) + 
...
feature_comparison_plot <- filtered_data %>%
  ...
  mutate(feature_set = case_when(
    feature_set == "fb" ~ "FB",
    feature_set == "fb_and_offline" ~ "FB and Offline",
...
gadm1 <- read_csv(here("data", "aggregated_indicators_lmic_july8.csv"), 
  ...
  janitor::clean_names() %>% 
  dplyr::select(country, gid_1, all_of(c(dhs_vars, fb_feats_2021, off_feats))) %>% ## the vectors of vars are defined in helper function
...
features <- tolower(c(fb_feats_2021, off_feats)) 
dependent_vars <- c(
...
for (dependent_var in dependent_vars) {
  ...
  cv_results <- perform_cross_validation_loco(data_subset, dependent_var, features, country_col = country_col)
...
var_importance_data <- cv_results %>%
  ...
  mutate(feature_recode = case_when(
    grepl("fb_pntr_13p_male_2021_impute", feature) ~ "Facebook Penetration (Male)",
    grepl("fb_pntr_13p_female_2021_impute", feature) ~ "Facebook Penetration (Female)",
    ...
    grepl("all_devices_age_13_plus_gg_2024_tess", feature) ~ "Facebook Penetration Gender Gap",
    ...
    feature == "gdi" ~ "Gender Development Index (National)",
    ...
    grepl("dhsyear", feature) ~ "DHS Survey Year",
    grepl("gdi_nation", feature) ~ "Gender Development Index",
...
ggsave(plot = var_importance_plot, filename = here("figures", "var_importance.png"), width = 26, height = 16, units = "in", dpi = 300)
```
...
ggsave(plot = var_importance_plot, filename = here( "figures", "variable_importance.png"), width = 13, height = 8, units = "in", dpi = 300)
```
...
#                                                  covar_list = c(fb_feats, off_feats_current),
