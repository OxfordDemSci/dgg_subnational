...
master_file_full <- read_csv(here("data", "master_data_file_dec27_with_national.csv")) %>% 
  filter(!is.na(fb_pntr_18p_female)) %>% 
  filter(year == 2024 & month == 11)

## read in predictions 
predictions_wide <- read_csv(here("out", "predictions_and_observed.csv")) 
...
shapefile_df <- gadm1_map %>% 
  left_join(predictions_uncertainty, by = c("GID_1" = "gid_1")) %>% 
    left_join(predictions_wide %>% filter(year == 2024 & month == 11), by = c("GID_1" = "gid_1")) 
...
internet_women_predicted_error_relative <- ggplot() +
  geom_sf(data = gadm1_map, fill = "grey70", color = "black", linewidth = 0.5) +
  geom_sf(data = shapefile_df, aes(fill = predicted_error / predicted_perc_used_internet_past12months_wght_age_15_to_49_wom), 
          color = "white", size = 0.01, alpha = 0.8, linewidth = 0.05) +
  scale_fill_gradientn(
    colors = viridis::magma(10), 
    values = scales::rescale(c(0, 0.1, 0.3, 0.6, 1.6)),
    breaks = c(0, 0.2, 0.5, 1, 1.5),
    labels = c("0", "0.2", "0.5", "1", "1.5"),
    na.value = "transparent",
    guide = guide_colorbar(
      barwidth = 20,
      barheight = 1,
...
error_dist_nigeria <- shapefile_df_nigeria %>% 
  ...
  geom_point(aes(size = pop_all_age_total), alpha = 0.3) +
  # geom_smooth() +
