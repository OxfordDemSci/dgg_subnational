...
time_series_plot <- change_df_metrics %>% 
  ...
  xlim(-.4, 1) + 
  ylim(-.4, 1)  + 
  labs(x = "Observed Change (Admin-1 Level)",
       y = "Predicted Change (Admin-1 Level)") + 
  theme(
...
df_nigeria <- predictions %>%
  ...
  filter(gid_0=="NGA", outcome=="internet_women") %>%
...
## create plot 
nigeria_plot <- df_nigeria %>% ggplot(aes(x = date)) +
  geom_line(aes(y = predicted, group = gid_1, color = "Predicted"), linewidth = 1.5) +
  geom_point(
    data = filter(df_nigeria, !is.na(survey_type)),
    aes(y = estimate, color = survey_type), size = 2, alpha = 1
  ) +
  scale_color_manual(
    values = c(
            Predicted = "lightblue",
    ...
    name = "Source",
    breaks = c("DHS", "LSMS", "MICS", "Predicted")
  ) +
  facet_wrap(~NAME_1) +
  labs(
    x = "Year",
...
#   )
# 
# # Plot national-level average
# nigeria_plot <- ggplot(df_nigeria_avg, aes(x = year)) +
#   geom_line(aes(y = predicted, color = "Predicted"), linewidth = 1.5) +
#   geom_point(
#     data = filter(df_nigeria_avg, !is.na(survey_type)),
#     aes(y = estimate, color = survey_type), size = 2
#   ) +
#   scale_color_manual(
#     values = c(
#       Predicted = "lightblue",
