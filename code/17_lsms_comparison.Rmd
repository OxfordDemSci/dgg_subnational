...
## Show gadm1 
dhs_ground_truth <- read_csv(here("data", "subnational_ground_truth_sep17_2024.csv"), locale = locale(encoding = "ISO-8859-1")) %>% 
...
```{r}
## LSMS Estimates 
ground_truth_lsms_long <- ground_truth_lsms %>%
  mutate(
  ...
  select(gid_0, gid_1, year, outcome, observed, sample_size)

## LSMS Estimates 
ground_truth_lsms_long <- ground_truth_lsms_long %>% 
  #filter(!(gid_0 == "NER" & year == 2021)) %>%
...
```{r}
## create table 
lsms_table <- ground_truth_lsms_long %>%
...
## print table 
print(xtable(lsms_table, digits = 0), include.rownames=FALSE, file = here("figures", "lsms_sample_table.tex"))
```
...
lsms_comparison_df <- predictions %>% 
  ...
  inner_join(ground_truth_lsms_long, by = c("year", "gid_1", "outcome")) %>% 
...
```{r}
## ground truth 
dhs_vs_lsms_df <- dhs_ground_truth %>% 
  left_join(ground_truth_lsms_long, by = c("gid_0", "gid_1", "outcome")) %>% 
  mutate(survey_diff = survey_year - year) %>% 
...
r_squared_results <- dhs_vs_lsms_df %>%
  group_by(outcome) %>%
  summarize(
    r = cor(observed, observed, use = "complete.obs"),  # Calculate correlation
    r2 = compute_r2(observed, observed),  # Calculate R-squared
    mae = mean(abs(observed - observed)), 
    .groups = "drop"
  )
...
lsms_vs_dhs <- dhs_vs_lsms_df %>% 
    left_join(r_squared_results, by = "outcome") %>%
  ggplot(aes(x = observed, y = observed, color = survey_diff)) + 
  geom_point(size = 2, alpha = 0.85) +
  geom_abline(color = "grey60", linetype = "dashed") +
  xlim(0, 1) + 
  ylim(0, 1) + 
