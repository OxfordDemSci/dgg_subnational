...
predictions_combined <- predictions_combined %>%
  left_join(gadm1, by = c("gid_1", "country")) #%>% # %>%  filter(country != "India") #  filter(continent != "Africa") 
...
## 
predictions_combined <- predictions_combined %>%
  left_join(model_performance_metrics %>% filter(metric == "r2"), by = c("cv_method", "model", "outcome", "feature_set")) %>%
  mutate(r2 = value)
```
...
plot1_observed_predicted <- predictions_combined %>%
  ...
  theme_cowplot() +
  labs(title = "Women") +
  geom_text(aes(label = paste("italic(R)^2 == ", round(r2, 2)), x = 65, y = 10), parse = TRUE, size = 3) +
...
plot3_observed_predicted <- predictions_combined %>%
 ...
 # stat_cor(method = "pearson", label.x = .65, label.y = .10, aes(label = after_stat(r.label))) +
  geom_text(aes(label = paste("italic(R)^2 == ", round(r2, 2)), x = .65, y = 0.1), parse = TRUE, size = 3) +
  facet_wrap(~outcome_categorical, nrow = 2) +
  theme(legend.position = "none") +
...
plot1_combined <- predictions_combined %>%
  ...
  theme_cowplot() +
  labs(title = "Women") +
  geom_text(aes(label = paste("italic(R)^2 == ", round(r2, 2)), x = 65, y = 10), parse = TRUE, size = 4) +
...
plot2_combined <- predictions_combined %>%
  ...
  geom_point(aes(size = pop_all_age_total_dhsyear), alpha = 0.2) +
  # geom_smooth() +
  ylim(0, 110) +
  xlim(0, 110) +
  geom_abline(slope = 1, linetype = "dashed", color = "grey") +
...
plot3_combined <- predictions_combined %>%
  ...
  theme_cowplot() +
  labs(title = "Gender Gap", size = "Population Size") +
  geom_text(aes(label = paste("italic(R)^2 == ", round(r2, 2)), x = .65, y = 0.1), parse = TRUE, size = 4) +
  facet_grid(rows = vars(outcome_categorical), cols = vars(cv_method)) +
  theme(legend.position = "none") +
  scale_size_continuous(
...
observed_predicted_plot_country_mobile_wom <- predictions_combined %>%
  ...
  ggplot(aes(x = observed, y = predicted)) +
  geom_point(alpha = 0.4, aes(size = pop_all_age_total_dhsyear)) +
  stat_cor(aes(x = observed, y = predicted, label = paste(after_stat(r.label)))) + # Adjusted stat_cor
  labs(
...
observed_predicted_plot_country_mobile_men <- predictions_combined %>%
  ...
  ggplot(aes(x = observed, y = predicted)) +
  geom_point(aes(size = pop_all_age_total_dhsyear), alpha = 0.4) +
  # geom_smooth() +
  stat_cor(aes(label = paste(after_stat(r.label)))) +
  labs(
