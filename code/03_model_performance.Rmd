...
for (i in seq_along(outcomes)) {
  ...
  plot_obj <- ggplot(df, aes(x = observed, y = predicted)) +
    ...
    geom_label(
      data = distinct(df, country, r, mae),
      aes(
        x = 0.1, y = 1.05,
        label = paste0("r==", round(r, 2), "*','~MAE==", round(mae, 2))
      ),
      parse = TRUE,
      inherit.aes = FALSE,
      size = 3,
    ...
    theme(
      ...
      panel.spacing = unit(.2, "lines"),
      aspect.ratio = .9,
      legend.position = ifelse(grepl("fm_ratio", outcome_var), "none", "bottom")
    )
...
## save plots
ggsave(plot = observed_predicted_plot_country_internet_wom, filename = here("figures", "observed_predicted_plot_country_internet_wom.png"), width = 10, height = 9)
ggsave(plot = observed_predicted_plot_country_internet_men, filename = here("figures", "observed_predicted_plot_country_internet_men.png"), width = 10, height = 9)
ggsave(plot = observed_predicted_plot_country_internet_fm_ratio, filename = here("figures", "observed_predicted_plot_country_internet_fm_ratio.png"), width = 10, height = 9)
```
